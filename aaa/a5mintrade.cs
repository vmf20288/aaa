...
//
// Nombre de clase/archivo: a5mintrade   (NO cambiar el nombre de la clase/archivo)
//
// Descripción (v6):
...
{
    public class a5mintrade : Indicator
    {
        ...
        private const string InternalName    = "a5mintrade";
        ...
        [Display(Name = "MinTrade", Order = 0, GroupName = "Parámetros")]
        public int MinTrade { get; set; }

        [NinjaScriptProperty]
        [Range(0, 200)]
        [Display(Name = "Tolerancia (ticks)", Order = 1, GroupName = "Parámetros")]
        public int ToleranciaTicks { get; set; }

        [NinjaScriptProperty]
        [Range(1, 100000)]
        [Display(Name = "Tiempo clúster (ms)", Order = 2, GroupName = "Parámetros")]
        public int ClusterWindowMs { get; set; }

        [NinjaScriptProperty]
        [Range(0, 1000)]
        [Display(Name = "Drift (± ticks)", Order = 3, GroupName = "Parámetros")]
        public int DriftTicks { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Usar 'al menos' (ON/OFF)", Order = 4, GroupName = "Parámetros")]
        public bool UseAlMenos { get; set; }

        [NinjaScriptProperty]
        [Range(1, int.MaxValue)]
        [Display(Name = "Al menos (vol)", Order = 5, GroupName = "Parámetros")]
        public int AlMenosMinTrade { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "Usar MinPrint (ON/OFF)", Order = 6, GroupName = "Parámetros")]
        ...
        private void DrawActiveRay(MinTradeLevel lv, Brush brush)
        {
            ...
            if (startBarsAgo == 0)
            {
                var activeRayTime = Draw.Ray(this, lv.TagLineActive, lv.TickTime, lv.Price, lv.TickTime.AddMilliseconds(1), lv.Price, brush);
                if (activeRayTime != null && activeRayTime.Stroke != null)
                    activeRayTime.Stroke.Width = 2;
                return;
            }
            ...
            var activeRayBars = Draw.Ray(this, lv.TagLineActive, startBarsAgo, lv.Price, endBarsAgo, lv.Price, brush);
            if (activeRayBars != null && activeRayBars.Stroke != null)
                activeRayBars.Stroke.Width = 2;
        }
        ...
        private void FreezeLineAtCurrent(MinTradeLevel lv)
        {
            ...
            if (startBarsAgo == 0)
            {
                ...
                var frozenLineTime = Draw.Line(this, lv.TagLineFrozen, false, lv.TickTime, lv.Price, lv.TickTime.AddMilliseconds(1), lv.Price, segBrush, DashStyleHelper.Solid, 2);
                if (frozenLineTime != null && frozenLineTime.Stroke != null)
                    frozenLineTime.Stroke.Width = 2;
                return;
            }
            ...
            // Firma con isAutoScale explícito para NT8
            var frozenLineBars = Draw.Line(this, lv.TagLineFrozen, false, startBarsAgo, lv.Price, endBarsAgo, lv.Price, segBrush, DashStyleHelper.Solid, 2);
            if (frozenLineBars != null && frozenLineBars.Stroke != null)
                frozenLineBars.Stroke.Width = 2;
        }
